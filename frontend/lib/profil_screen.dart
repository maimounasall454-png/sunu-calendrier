...
class _ProfilScreenState extends State<ProfilScreen> {
  bool _connecte = false; String _username = ''; bool _loading = false;
  final _uCtrl = TextEditingController(); final _pCtrl = TextEditingController();
  bool _showPass = false;
  @override void initState() { super.initState(); _verifier(); }
 
  Future<void> _verifier() async {
    final ok = await ApiService.estConnecte();
    final prefs = await SharedPreferences.getInstance();
    setState(() { _connecte = ok; _username = prefs.getString('username') ?? ''; });
  }
  ...
  Widget _profil() => Column(mainAxisAlignment: MainAxisAlignment.center, children: [
    const CircleAvatar(radius:48, backgroundColor: Color(0xFFFFEBEE),
      child: Icon(Icons.person, size:48, color: Color(0xFFB71C1C))),
    ...
    SizedBox(width: double.infinity, child: ElevatedButton.icon(
      ...
      onPressed: () async { await ApiService.deconnexion(); _verifier(); })),
  ]);
 
  Widget _connexionForm() => Column(mainAxisAlignment: MainAxisAlignment.center, children: [
    const Icon(Icons.lock_outline, size:64, color: Color(0xFFB71C1C)),
    const SizedBox(height:16),
    const Text('Connexion', style: TextStyle(fontSize:26, fontWeight:FontWeight.bold)),
    const SizedBox(height:28),
    TextField(controller: _uCtrl,
      decoration: const InputDecoration(labelText:'Nom d\'utilisateur', prefixIcon: Icon(Icons.person), border: OutlineInputBorder())),
    const SizedBox(height:14),
    TextField(controller: _pCtrl, obscureText: !_showPass, decoration: InputDecoration(
      labelText:'Mot de passe', prefixIcon: const Icon(Icons.lock), border: const OutlineInputBorder(),
    ...
    const Text('Utilisez le compte créé par M1 (admin / admin123)', style: TextStyle(color:Colors.grey, fontSize:12)),
  ]);
}
